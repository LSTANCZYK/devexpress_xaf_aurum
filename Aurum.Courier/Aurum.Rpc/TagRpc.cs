...
namespace Aurum.Courier.Rpc
{
    public class TagRpc : ITagRpc
    {
        ...
        public virtual Task<TResponse> Request<TRequest, TResponse>(string tag, TRequest request)
            ...
            where TResponse : class
        {
            ...
            var routingKey = GetTaggedRpcRoutingKey(conventions.RpcRoutingKeyNamingConvention(typeof(TRequest)), tag);
        ...
        public virtual IDisposable Respond<TRequest, TResponse>(string tag, Func<TRequest, Task<TResponse>> responder)
            where TRequest : class
            where TResponse : class
        {
            return Respond(tag, responder, c => { });
        }

        public virtual IDisposable Respond<TRequest, TResponse>(string tag, Func<TRequest, Task<TResponse>> responder, Action<IResponderConfiguration> configure)
            where TRequest : class
            where TResponse : class
        {
            Preconditions.CheckNotBlank(tag, "tag");
        ...
        private static string GetTaggedRpcRoutingKey(string routingKey, string tag)
        {
            return routingKey + "_" + tag;
        }
    }
}
